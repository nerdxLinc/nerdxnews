...
// .wrangler/tmp/pages-89Uh8N/functionsWorker-0.3935885837960562.mjs
...
var onRequest = /* @__PURE__ */ __name2(async (context) => {
  ...
  if (request.method === "GET") {
    ...
    try {
      if (slug) {
        const row = admin ? await db.prepare(
          `
                SELECT *
                FROM posts
                WHERE slug = ?
                LIMIT 1
              `
        ).bind(slug).first() : await db.prepare(
          `
                SELECT *
                FROM posts
                WHERE slug = ?
  ...
  if (request.method === "POST") {
    try {
      ...
      if (featuredFlag === 1) {
        await db.prepare(`UPDATE posts SET isFeatured = 0 WHERE isFeatured = 1`).run();
      }
      if (cleanId) {
        const existingById = await db.prepare(`SELECT id, slug FROM posts WHERE id = ? LIMIT 1`).bind(cleanId).first();
        if (existingById) {
          const slugClash = await db.prepare(
            `SELECT id FROM posts WHERE slug = ? AND id != ? LIMIT 1`
          ).bind(cleanSlug, cleanId).first();
          if (slugClash) {
            return json(
...
// .wrangler/tmp/pages-89Uh8N/izb3xl7s92l.js
...
// .wrangler/tmp/bundle-LBPJ8h/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-LBPJ8h/middleware-loader.entry.ts
...
//# sourceMappingURL=izb3xl7s92l.js.map
